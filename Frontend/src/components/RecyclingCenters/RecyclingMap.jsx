...
const addMarkers = () => {
  ...
  async (pos) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:5000/api/recycling-centers/centers/nearby?lat=${lat}&lng=${lng}&distance=${maxDistance}`
      );
      const data = await response.json();
      setFiltered(data);
  ...
  try {
    ...
    const backendResponse = await fetch(`http://localhost:5000/api/centers/nearby?lat=${lat}&lng=${lng}&distance=${maxDistance}`);
